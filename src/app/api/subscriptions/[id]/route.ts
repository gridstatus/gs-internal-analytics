...
import { getSubscriptionById, toSubscriptionDetail, loadSql } from '@/lib/queries';
...
export async function GET(
  ...
  return withRequestContext(searchParams, async () => {
    try {
      ...
      const subscription = toSubscriptionDetail(rows[0]);
...
export async function DELETE(
  ...
  try {
    ...
    const row = rows[0];
    if (row.stripe_subscription_id != null && row.stripe_subscription_id !== '') {
      return NextResponse.json(
        { error: 'Cannot delete a subscription that is linked to Stripe. Remove the Stripe link first or cancel in Stripe.' },
        { status: 400 }
      );
    }

    const sql = loadSql('mutations/subscription-delete.sql');
    if (preview) {
      const interpolated = sql.replace(/\$1/g, String(subscriptionId));
      const displaySql = formatSql(interpolated, { language: 'postgresql', tabWidth: 2 });
      return NextResponse.json({ sql: displaySql });
    }

    await writeQuery(sql, [subscriptionId]);
